...
const addInfo = async function(req, res) {
    ...
    User.findOneOne({ username: artistName}, function (err, user) {
        if (err) {
            console.error(err);
        } else {
    ...
    User.updateOne({username: artistName}, {$set: { bio: newArtistBio }} , function(err) {
        if (err) {
...
const deleteInfo = async function(req, res) {
    ...
    User.updateOne({username: artistName}, {$set: { bio: "" }} , function(err) {
        if (err) {
