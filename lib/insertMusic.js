...
const addNewAlbum = async function (req, res) {
    ...
    album.save(function (err) {
        if (err) {
            ...
            res.status(500).json({ error: 'Error on saving the album to the db, please retry' });
            return;
        } else {
...
// NEEDS METHODS FOR CHECKING AUTHENTICATION (Sprint #2)
const deleteAlbum = function (req, res) {
...
const changeAlbumData = async function (req, res) {
    ...
    if (!isNaN(parseInt(req.body.year))) {
        ...
        message = message + "\nNew year is not valid (must be a number); ";
    }
    ...
    Album.findOneAndUpdate({owner: artist, ismn: oldIsmn}, newData, function (err, album) {
        if(err){
            ...
            if (message == "\nWARNING(s): "){
                res.status(200).json({ message: "The album data has been correctly changed in the db", newIsmn: target});
                return;
            } else {
