...
//           - incremental ID
//           - change album data needs complet data each time, should instead get old data when ne data is incomplete
...
const addNewAlbum = (req, res) => {
    console.log("new add album request from "+req.protocol + '://' + req.get('host') + req.originalUrl);
...
const changeAlbumData = (req, res) => {
    console.log("new change album request from "+req.protocol + '://' + req.get('host') + req.originalUrl);
    let artist = req.params.name;
    let ismn = parseInt(req.params.ismn);
